               ...
               bool verbose)
{
    ...
    for (int i = 1; i < burn_in + N; i++) {
        ...
        for(int j = 0; j < p; j++) {
			...
			alpha(i, j) = rtbeta(a[j] + sum1[j], 1 + sum2[j], 1 - sum_minus_alpha);		
		}
...
int m_full_conditional_pyinar(double alpha, double lambda, int yt_i, int yt, IntegerVector m_minus_t) {
    int M = m_minus_t.size(), sum_m_minus_t;
...
double lambda_full_conditional_pyinar(NumericVector minus_lambda_t, double a0, double b0, double tau, double sigma, int k_minus_lambda_t, IntegerVector n_minus_t, int y_t, int sum_m) {
    ...
    weight_gamma = exp( log(tau + k_minus_lambda_t*sigma) + a0*log(b0) + lgamma(y_t - sum_m + a0) - lgamma(a0) - (y_t - sum_m + a0)*log(b0 + 1) ); 	
...
// [[Rcpp::export(.posterior_pyinar)]]
List posterior_pyinar(IntegerVector y,  
			   int p,
			   List prior, 
			   int burn_in, 
			   int N,
...
{	
    ...
    for (int i = 1; i < burn_in + N; i++) {
        ...
        for (int t = 0; t < T - p; t++) {
			...
			sum_m = 0; for(int j = 0; j < p; j++)  sum_m += m(t, j);
            lambda(i, t) = lambda_full_conditional_pyinar(minus_lambda_t, a0, b0, tau, sigma, k_minus_lambda_t, n_minus_t, y[t + p], sum_m);
        }
    
       for (int j = 0; j < p; j++) {
            for (int t = 0; t < T - p; t++) {
                minus_m = minus_m_ij(m, t, j);
                m(t, j) = m_full_conditional_pyinar(alpha(i, j), lambda(i, t), y[t + p - j - 1], y[t + p], minus_m);
            }
        }
        NumericVector lambda_star = unique(lambda.row(i));
        int k = lambda_star.size();
...
double polya_blackwell_macqueen_pyinar(double a0, double b0, double tau, double sigma, NumericVector lambda) {
    int t = lambda.size();
    double u = R::runif(0, 1);
...
// [[Rcpp::export(.predictive_distribution_pyinar)]]
NumericVector predictive_distribution_pyinar(List model, int h, int replications) {
    ...
    int N = chain["length"];
    List prior = model["prior"];
    double a0 = prior["a0"], b0 = prior["b0"], sigma = prior["sigma"], tau = prior["tau"];
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int r = 1; r <= h; r++) {
            NumericVector given_lambda(T - p - 1 + r);
            for (int s = 0; s < T - p - 1 + r; s++) given_lambda[s] = lambda(i, s);
            lambda(i, T - p - 1 + r) = polya_blackwell_macqueen_pyinar(a0, b0, tau, sigma, given_lambda);
        }
    }
